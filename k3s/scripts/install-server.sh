...
[ "${DISABLE_FLANNEL:-false}" = "true" ] && INSTALL_ARGS+=(--flannel-backend none)
[ "${DISABLE_KUBE_PROXY:-false}" = "true" ] && INSTALL_ARGS+=(--disable-kube-proxy)
...
echo "  sed -i 's/127.0.0.1/${NODE_IP}/g' ~/.kube/config"
